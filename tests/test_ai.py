...
def sample_expenses():
    return [
        {"amount": 3000.0, "category": "Food", "description": "Groceries", "date": "2026-07-01"},
        {"amount": 1500.0, "category": "Transport", "description": "Taxi", "date": "2026-07-10"},
    ]


# --- predict_monthly_spending ---

def test_predict_monthly_spending_projects_based_on_daily_pace(fixed_today, sample_budget_status):
    projected = ai.predict_monthly_spending(sample_budget_status)
    assert projected == 9000.0

def test_predict_monthly_spending_on_day_one_multiplies_spent_by_30(monkeypatch, sample_budget_status):
    monkeypatch.setattr(ai.datetime, "date", type("D1", (datetime.date,), {
...
def test_build_financial_context_includes_budget_numbers(sample_expenses, sample_budget_status):
...
def test_build_financial_context_includes_each_transaction(sample_expenses, sample_budget_status):
    context = ai.build_financial_context(sample_expenses, sample_budget_status)
...
def test_build_financial_context_empty_expenses_says_so_explicitly(sample_budget_status):
